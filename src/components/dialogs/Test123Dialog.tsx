...
interface Test123DialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Test123['fields']) => Promise<void>;
  defaultValues?: Test123['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function Test123Dialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: Test123DialogProps) {
  const [fields, setFields] = useState<Partial<Test123['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'test_123');
      await onSubmit(clean as Test123['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Test123['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Test 123 bearbeiten' : 'Test 123 hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              />
              <Label htmlFor="aktiv" className="font-normal">Aktiv</Label>
            </div>
          </div>
          <DialogFooter>
