...
import type { Test123, CreateTest123 } from '@/types/app';
...
export class LivingAppsService {
  // --- TEST_123 ---
  static async getTest123(): Promise<Test123[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TEST_123}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Test123[];
    return enrichLookupFields(records, 'test_123');
  }
  static async getTest123Entry(id: string): Promise<Test123 | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TEST_123}/records/${id}`);
    const record = { record_id: data.id, ...data } as Test123;
    return enrichLookupFields([record], 'test_123')[0];
  }
  static async createTest123Entry(fields: CreateTest123) {
    return callApi('POST', `/apps/${APP_IDS.TEST_123}/records`, { fields: cleanFieldsForApi(fields as any, 'test_123') });
  }
  static async updateTest123Entry(id: string, fields: Partial<CreateTest123>) {
    return callApi('PATCH', `/apps/${APP_IDS.TEST_123}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'test_123') });
  }
  static async deleteTest123Entry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TEST_123}/records/${id}`);
  }
