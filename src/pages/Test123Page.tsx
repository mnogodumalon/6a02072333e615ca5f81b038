...
export default function Test123Page() {
  const [records, setRecords] = useState<Test123[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Test123 | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Test123 | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Test123 | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getTest123());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Test123['fields']) {
    await LivingAppsService.createTest123Entry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Test123['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTest123Entry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Test 123"
      subtitle={`${records.length} Test 123 im System`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Test 123. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
