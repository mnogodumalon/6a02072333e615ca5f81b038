...
import { Test123ViewDialog } from '@/components/dialogs/Test123ViewDialog';
...
const TEST123_FIELDS = [
  ...
  { key: 'aktiv', label: 'Aktiv', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'test_123', label: 'Test 123', pascal: 'Test123' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('test_123');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'test_123': return (data as any).test123 as Test123[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'test_123': return TEST123_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'test_123': return {
        create: (fields: any) => LivingAppsService.createTest123Entry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTest123Entry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTest123Entry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'test_123' || dialogState?.entity === 'test_123') && (
        <Test123Dialog
          open={createEntity === 'test_123' || dialogState?.entity === 'test_123'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'test_123' ? handleUpdate : (fields: any) => handleCreate('test_123', fields)}
          defaultValues={dialogState?.entity === 'test_123' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Test123']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Test123']}
        />
      )}
      {viewState?.entity === 'test_123' && (
        <Test123ViewDialog
          open={viewState?.entity === 'test_123'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'test_123', record: r }); }}
        />
      )}
